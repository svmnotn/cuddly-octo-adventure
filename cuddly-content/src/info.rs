...
pub fn build(tabs: &gtk::Notebook) {
    ...
    text_input!(cont,
                ARCHIVE.read().unwrap().info.name,
                ARCHIVE.write().unwrap().info.name,
                "Name: ",
                "The human readable name of the Cuddle.");

    text_input!(cont,
                ARCHIVE.read().unwrap().info.description,
                ARCHIVE.write().unwrap().info.description,
                "Description: ",
                "The description of the content of this cuddle.\n\
       Ex: Physics questions for all.");

    text_input!(cont,
                ARCHIVE.read().unwrap().info.version,
                ARCHIVE.write().unwrap().info.version,
                "Version: ",
    ...
    text_input!(cont,
                ARCHIVE.read().unwrap().info.author,
                ARCHIVE.write().unwrap().info.author,
                "Author: ",
                "The human that made this cuddle and preferibly where to contact them.\n\
      Ex: Sir Issac Newton <issacnewton@email.com>");

    text_input!(cont,
                ARCHIVE.read().unwrap().info.license,
                ARCHIVE.write().unwrap().info.license,
                "License: ",
                "The name of the license under which this cuddle is created.");

    text_input!(cont,
                ARCHIVE.read().unwrap().info.update_url,
                ARCHIVE.write().unwrap().info.update_url,
                "Update URL: ",
